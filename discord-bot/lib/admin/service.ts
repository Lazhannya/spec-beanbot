...
export interface HealthCheck {
  ...
  checks: {
    ...
    storage: { status: "ok" | "warning" | "error"; usage: number; message?: string };
    sessions: { status: "ok" | "warning"; count: number; message?: string };
    reminders: { status: "ok" | "warning"; activeCount: number; message?: string };
  };
  warnings: string[];
  errors: string[];
}
...
export class AdminService {
  ...
  static async getSystemStats(): Promise<SystemStats> {
    try {
      ...
      const stats: SystemStats = {
        ...
        // Response metrics
        totalAcknowledgments,
        acknowledgmentRate: totalDeliveries > 0 ? (totalAcknowledgments / totalDeliveries) * 100 : 0,
  ...
  static async getAllUsers(options: {
    ...
    try {
      const kv = await getKV();
      const { limit = 50, offset = 0, sortBy = "lastSeen", sortOrder = "desc" } = options;
      ...
      for (const [userId, user] of userMap.entries()) {
        const userActivity = await ReminderHistoryService.generateUserSummary(userId, 30);
  ...
  static async performHealthCheck(): Promise<HealthCheck> {
    ...
    try {
      ...
      result.checks.database.message = error instanceof Error ? error.message : "Database connection failed";
  ...
  static async performCleanup(): Promise<{
    ...
    try {
      ...
      result.errors.push(`History cleanup failed: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
