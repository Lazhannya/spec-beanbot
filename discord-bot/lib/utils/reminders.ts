...
export function createReminderFromInput(
  ...
  const nextDeliveryAt = calculateNextDelivery(scheduleWithDefaults, input.timezone);
 ...
 */
function calculateOnceDelivery(schedule: ReminderSchedule, baseDate: Date): Date | undefined {
 ...
 */
function calculateDailyDelivery(schedule: ReminderSchedule, baseDate: Date): Date | undefined {
  const [hours, minutes] = schedule.time.split(":").map(Number);
 ...
 */
function calculateWeeklyDelivery(schedule: ReminderSchedule, baseDate: Date): Date | undefined {
 ...
 */
function calculateMonthlyDelivery(schedule: ReminderSchedule, baseDate: Date): Date | undefined {
  const [hours, minutes] = schedule.time.split(":").map(Number);
 ...
 */
function calculateYearlyDelivery(schedule: ReminderSchedule, baseDate: Date): Date | undefined {
  const [hours, minutes] = schedule.time.split(":").map(Number);
 ...
 */
function calculateIntervalDelivery(schedule: ReminderSchedule, baseDate: Date): Date | undefined {
 ...
 */
function calculateCustomDelivery(_schedule: ReminderSchedule, _baseDate: Date): Date | undefined {
...
export function updateReminderAfterDelivery(reminder: Reminder): Reminder {
  ...
  const nextDelivery = calculateNextDelivery(updatedSchedule, reminder.timezone, now);
...
export function formatReminderForDisplay(reminder: Reminder): string {
  ...
  const timeZoneText = reminder.timezone !== "UTC" ? ` (${reminder.timezone})` : "";
  ...
  switch (schedule.type) {
    case "once":
      return schedule.startDate 
        ? `Once on ${new Date(schedule.startDate).toLocaleDateString()} at ${time}`
        : `Once at ${time}`;
    ...
    case "monthly": {
      const dayText = schedule.dayOfMonth ? `${schedule.dayOfMonth}` : "same day";
    ...
    case "interval": {
      const intervalText = schedule.interval === 1 ? "day" : `${schedule.interval} days`;
 ...
 */
export function checkReminderConflict(reminder1: Reminder, reminder2: Reminder): boolean {
